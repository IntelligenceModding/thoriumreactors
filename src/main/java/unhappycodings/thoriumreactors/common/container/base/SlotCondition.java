...
public class SlotCondition {
    ...
    public List<Predicate<ItemStack>> neededPreds = new ArrayList<>();
    ...
    public SlotCondition setNeededPred(Predicate<ItemStack> neededPred) {
        this.neededPreds.clear();
        this.neededPreds.add(neededPred);
        return this;
    }
    ...
    public boolean isValid(ItemStack stack) {
        ...
        if (!neededPreds.isEmpty()) {
            for (Predicate<ItemStack> predicate : neededPreds) {
                if (!predicate.test(stack)) valid = false;
            }
        }
        return valid;
    }
