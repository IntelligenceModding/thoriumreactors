...
public class ThoriumChestScreen extends BaseScreen<ThoriumChestContainer> {
    ...
    protected void renderLabels(@NotNull GuiGraphics graphics, int pMouseX, int pMouseY) {
        ...
        ScreenUtil.drawText(Component.translatable("block.thoriumreactors.thorium_chest_block").withStyle(ScreenUtil::notoSans), graphics, 10, 2, 11184810);
        ScreenUtil.drawRightboundText(Component.literal(Minecraft.getInstance().player.getScoreboardName()).withStyle(ScreenUtil::notoSans), graphics, 343, 2, 11184810);
        pPoseStack.popPose();
        ScreenUtil.drawText(Component.translatable("key.categories.inventory").withStyle(ScreenUtil::notoSans), graphics, 44, 163, 11184810);
    }
