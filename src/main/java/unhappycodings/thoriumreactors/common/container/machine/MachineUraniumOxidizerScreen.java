...
public class MachineUraniumOxidizerScreen extends MachineScreen<MachineUraniumOxidizerContainer> {
    ...
    protected void renderBg(@NotNull PoseStack matrixStack, float partialTicks, int x, int y) {
        ...
        blit(matrixStack, getGuiLeft() + 37, getGuiTop() + 44, 181, 52, 4, 63); // Left Tank
        ...
        blit(matrixStack, getGuiLeft() + 70, getGuiTop() + 52, 184, 0, width, 9); // Right Tank
    ...
    protected void renderLabels(@NotNull PoseStack pPoseStack, int pMouseX, int pMouseY) {
        ...
        RenderUtil.drawText(Component.translatable(FormattingUtil.getTranslatable("machines.oxidizer.name")).withStyle(RenderUtil::notoSans), pPoseStack, 10, 2, 11184810);
        ...
        if (RenderUtil.mouseInArea(getGuiLeft() + 37, getGuiTop() + 42, getGuiLeft() + 54, getGuiTop() + 85, pMouseX, pMouseY))
        ...
        if (RenderUtil.mouseInArea(getGuiLeft() + 34, getGuiTop() + 90, getGuiLeft() + 36, getGuiTop() + 97, pMouseX, pMouseY))
            appendHoverText(pPoseStack, pMouseX, pMouseY, new Component[]{Component.translatable(FormattingUtil.getTranslatable("machines.tooltip.autodump")).append(" " + entity.isInputDump()), Component.translatable(FormattingUtil.getTranslatable("machines.tooltip.only_while_running"))});
        if (RenderUtil.mouseInArea(getGuiLeft() + 116, getGuiTop() + 90, getGuiLeft() + 118, getGuiTop() + 97, pMouseX, pMouseY))
            appendHoverText(pPoseStack, pMouseX, pMouseY, new Component[]{Component.translatable(FormattingUtil.getTranslatable("machines.tooltip.autodump")).append(" " + entity.isOutputDump()), Component.translatable(FormattingUtil.getTranslatable("machines.tooltip.only_while_running"))});
        if (RenderUtil.mouseInArea(getGuiLeft() + 37, getGuiTop() + 90, getGuiLeft() + 55, getGuiTop() + 97, pMouseX, pMouseY) || RenderUtil.mouseInArea(getGuiLeft() + 119, getGuiTop() + 90, getGuiLeft() + 137, getGuiTop() + 97, pMouseX, pMouseY))
            appendHoverText(pPoseStack, pMouseX, pMouseY, new Component[]{Component.translatable(FormattingUtil.getTranslatable("machines.tooltip.dumpinstantly"))});
