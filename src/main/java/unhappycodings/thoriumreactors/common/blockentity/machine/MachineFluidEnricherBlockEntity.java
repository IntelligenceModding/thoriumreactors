...
public class MachineFluidEnricherBlockEntity extends MachineContainerBlockEntity implements WorldlyContainer, MenuProvider, IEnergyCapable {
    ...
    public MachineFluidEnricherBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.FLUID_ENRICHER_BLOCK.get(), pPos, pBlockState);
    ...
    public @NotNull <T> LazyOptional<T> getCapability(@NotNull Capability<T> cap, @Nullable Direction side) {
        Direction facing = this.getBlockState().getValue(MachineFluidEnricherBlock.FACING);
        if (cap == ForgeCapabilities.ENERGY && supportsEnergy() && side != null && this.getBlockState().getValue(MachineFluidEnricherBlock.FACING).getOpposite() == side)
    ...
    public boolean canInputEnergy(Direction direction) {
        return this.getBlockState().getValue(MachineFluidEnricherBlock.FACING).getOpposite() == direction;
    }
    ...
    public void tick() {
        ...
        if (getState() && getRecipeTime() % 20 == 0) {
            this.level.playSound(null, getBlockPos(), ModSounds.MACHINE_URANIUM_OXIDIZER.get(), SoundSource.BLOCKS, 0.3f, 1.1f);
            this.level.playSound(null, getBlockPos(), ModSounds.MACHINE_CRYSTALIZER.get(), SoundSource.BLOCKS, 0.3f, 0.9f);
        }
    ...
    public boolean hasRecipeNeeds(int energy) {
        ...
        List<FluidEnrichingRecipe> recipe = level.getRecipeManager().getAllRecipesFor(ModRecipes.FLUID_ENRICHING_RECIPE_TYPE.get());
    ...
    public boolean getState() {
        return getBlockState().getValue(MachineFluidEnricherBlock.POWERED);
    }

    public void setState(boolean state) {
        level.setBlock(getBlockPos(), getBlockState().setValue(MachineFluidEnricherBlock.POWERED, state), 3);
    }
    ...
    protected AbstractContainerMenu createMenu(int pContainerId, @NotNull Inventory pInventory) {
        return new MachineFluidEnricherContainer(pContainerId, pInventory, getBlockPos(), getLevel(), getContainerSize());
    }
