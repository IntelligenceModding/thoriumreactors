...
public class MachineUraniumOxidizerBlockEntity extends MachineContainerBlockEntity implements WorldlyContainer, MenuProvider, IEnergyCapable {
    ...
    public MachineUraniumOxidizerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.URANIUM_OXIDIZER_BLOCK.get(), pPos, pBlockState);
    ...
    public @NotNull <T> LazyOptional<T> getCapability(@NotNull Capability<T> cap, @Nullable Direction side) {
        Direction facing = this.getBlockState().getValue(MachineUraniumOxidizerBlock.FACING);
        if (cap == ForgeCapabilities.ENERGY && supportsEnergy() && side != null && this.getBlockState().getValue(MachineUraniumOxidizerBlock.FACING).getOpposite() == side)
    ...
    public boolean canInputEnergy(Direction direction) {
        return this.getBlockState().getValue(MachineUraniumOxidizerBlock.FACING).getOpposite() == direction;
    }
    ...
    public void tick() {
        ...
        if (getState() && getRecipeTime() % 20 == 0) {
            this.level.playSound(null, getBlockPos(), ModSounds.MACHINE_URANIUM_OXIDIZER.get(), SoundSource.BLOCKS, 0.3f, 1f);
        }
    ...
    public boolean hasRecipeNeeds(int energy) {
        ...
        List<OxidizingRecipe> recipe = level.getRecipeManager().getAllRecipesFor(ModRecipes.OXIDIZING_RECIPE_TYPE.get());
        for (OxidizingRecipe decomposingRecipe : recipe) {
    ...
    public boolean getState() {
        return getBlockState().getValue(MachineUraniumOxidizerBlock.POWERED);
    }

    public void setState(boolean state) {
        level.setBlock(getBlockPos(), getBlockState().setValue(MachineUraniumOxidizerBlock.POWERED, state), 3);
    }
    ...
    protected AbstractContainerMenu createMenu(int pContainerId, @NotNull Inventory pInventory) {
        return new MachineUraniumOxidizerContainer(pContainerId, pInventory, getBlockPos(), getLevel(), getContainerSize());
    }
