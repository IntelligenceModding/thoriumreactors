    ...
    public void addProbeInfo(ProbeMode probeMode, IProbeInfo iProbeInfo, Player player, Level level, BlockState blockState, IProbeHitData iProbeHitData) {
        ...
        if (level.getBlockEntity(iProbeHitData.getPos()) instanceof FluidTankBlockEntity) {
            ...
            if (!entity.isActivated()) {
                iProbeInfo.mcText(Component.literal("Turbine Inactive").withStyle(FormattingUtil.hex(0x9F0006)));
                return;
            } else {
                iProbeInfo.mcText(Component.literal("Turbine Active").withStyle(FormattingUtil.hex(0x00A90B)));
            }
            if (level.getBlockEntity(entity.getPowerPortPos()) instanceof TurbinePowerPortBlockEntity powerPortBlockEntity)
                iProbeInfo.horizontal(iProbeInfo.defaultLayoutStyle().alignment(ElementAlignment.ALIGN_CENTER)).mcText(Component.literal("Energy: ")).progress(powerPortBlockEntity.getEnergy(), powerPortBlockEntity.getEnergyCapacity(), iProbeInfo.defaultProgressStyle().filledColor(COLOR_A).alternateFilledColor(COLOR_B).backgroundColor(Color.rgb(255, 255, 255, 255)).borderColor(Color.rgb(65, 65, 65, 255)).numberFormat(NumberFormat.COMMAS).suffix(" FE").showText(true));
            ...
            iProbeInfo.mcText(Component.literal("Producing: ").withStyle(FormattingUtil.hex(0x7ED355)).append(Component.literal(entity.isCoilsEngaged() ? FormattingUtil.formatEnergy(entity.getRpm() * (FormattingUtil.getTurbineGenerationModifier(entity.getRpm()))) : "0 FE" + "/t").withStyle(ChatFormatting.WHITE)));
            iProbeInfo.mcText(Component.literal("Speed: ").withStyle(FormattingUtil.hex(0x55D38A)).append(Component.literal(Math.floor(entity.getRpm() * 100) / 100 + " Rpm").withStyle(ChatFormatting.WHITE)));
            iProbeInfo.mcText(Component.literal("Flowrate: ").withStyle(FormattingUtil.hex(0x0ACECE)).append(Component.literal(entity.getCurrentFlowrate() + " mB/t").withStyle(ChatFormatting.WHITE)));
            iProbeInfo.mcText(Component.literal("Best performing @ 1900-2050 Rpm").withStyle(FormattingUtil.hex(0x55D38A)));
            ...
            if (!entity.isReactorActive()) {
                iProbeInfo.mcText(Component.literal("Reactor Inactive").withStyle(FormattingUtil.hex(0x9F0006)));
                return;
            } else {
                iProbeInfo.mcText(Component.literal("Reactor Active").withStyle(FormattingUtil.hex(0x00A90B)));
            }
            ...
            iProbeInfo.mcText(Component.literal("Rod Insert: ").withStyle(FormattingUtil.hex(0x7ED355)).append(Component.literal((int) ((controlValue / 6400f) * 100) + "%").withStyle(ChatFormatting.WHITE)));
            iProbeInfo.mcText(Component.literal("Uranium Fuel: ").withStyle(FormattingUtil.hex(0x55D38A)).append(Component.literal((int) ((fuelValue / 8100f) * 100) + "%").withStyle(ChatFormatting.WHITE)));
            iProbeInfo.mcText(Component.literal("Reactor Load: ").withStyle(FormattingUtil.hex(0x0ACECE)).append(Component.literal((int) (((entity.getReactorCurrentTemperature() - 22) / 949) * 100) + "%").withStyle(ChatFormatting.WHITE)));
            iProbeInfo.mcText(Component.literal("Status: ").withStyle(FormattingUtil.hex(0x0ACECE)).append(Component.literal(Math.floor(entity.getReactorStatus() * 10) / 10 + "%").withStyle(ChatFormatting.WHITE)));
            ...
            for (Direction value : Direction.values()) {
                level.getBlockEntity(iProbeHitData.getPos()).getCapability(ForgeCapabilities.ENERGY, value).ifPresent(storage -> {
                    iProbeInfo.horizontal(iProbeInfo.defaultLayoutStyle().alignment(ElementAlignment.ALIGN_CENTER)).mcText(Component.literal("Energy: ")).progress(storage.getEnergyStored(), storage.getMaxEnergyStored(), iProbeInfo.defaultProgressStyle().filledColor(COLOR_A).alternateFilledColor(COLOR_B).backgroundColor(Color.rgb(255, 255, 255, 255)).borderColor(Color.rgb(65, 65, 65, 255)).numberFormat(NumberFormat.COMMAS).suffix(" FE").showText(true));
                });
            }
            ...
            if (level.getBlockEntity(iProbeHitData.getPos()) instanceof MachineContainerBlockEntity containerBlockEntity) {
                ...
                if (tag.getInt("RecipeTime") != 0 && tag.getInt("MaxRecipeTime") != 0) {
                    iProbeInfo.mcText(Component.literal("Recipe: ").withStyle(FormattingUtil.hex(0x7ED355)).append(Component.literal(FormattingUtil.formatPercentNum(tag.getInt("MaxRecipeTime") - tag.getInt("RecipeTime"), tag.getInt("MaxRecipeTime"), false)).withStyle(ChatFormatting.WHITE)));
                }
                if (tag.getInt("Production") != 0 && tag.getInt("Fuel") != 0) {
                    ...
                    iProbeInfo.mcText(Component.literal("Producing: ").withStyle(FormattingUtil.hex(0x7ED355)).append(Component.literal(FormattingUtil.formatEnergy(tag.getInt("Production")) + "/t").withStyle(ChatFormatting.WHITE)));
                    iProbeInfo.mcText(Component.literal("Fuel: ").withStyle(FormattingUtil.hex(0x55D38A)).append(Component.literal(format.format(fuel)).withStyle(ChatFormatting.WHITE)));

                }
            }

            if (level.getBlockEntity(iProbeHitData.getPos()) instanceof ThermalValveBlockEntity entity) {
                iProbeInfo.mcText(Component.literal("Type: ").withStyle(FormattingUtil.hex(0x7ED355)).append(Component.translatable(entity.getBlockState().getValue(ThermalValveBlock.TYPE).getSerializedName()).withStyle(ChatFormatting.WHITE)));
            }

            if (level.getBlockEntity(iProbeHitData.getPos()) instanceof ReactorValveBlockEntity entity) {
                iProbeInfo.mcText(Component.literal("Type: ").withStyle(FormattingUtil.hex(0x7ED355)).append(Component.translatable(entity.getBlockState().getValue(ReactorValveBlock.TYPE).getSerializedName()).withStyle(ChatFormatting.WHITE)));
            }
